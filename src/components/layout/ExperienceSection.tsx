          ...
          {experiences.map((exp, index) => (
            ...
            >
              ...
              <div className="experience-timeline-node flex items-center justify-center">
                <Briefcase className="w-3.5 h-3.5 text-[#E8B94A] hover:text-[#FFF6F1] transition-colors" />
              </div>

              {/* Card Container */}
              <div className="group relative rounded-xl border border-[#E8E2DE] bg-[#FFFFFF] p-6 shadow-sm transition-all duration-400 hover:shadow-md hover:border-[#D4A843]/50">
                
                {/* Glowing edge on hover */}
                <div className="absolute left-0 top-0 bottom-0 w-[4px] rounded-l-xl bg-transparent transition-all group-hover:bg-[#D4A843]"></div>

                {/* Card Header info */}
                <div className="flex flex-col justify-between gap-2 md:flex-row md:items-center">
                  <div>
                    <h3 className="font-serif text-2xl font-bold text-[#573D00] group-hover:text-[#D4A843] transition-colors">
                      {exp.role}
                    </h3>
                    <p className="font-sans text-sm font-semibold tracking-wide text-[#8B7355] uppercase mt-1">
                      {exp.company}
                    </p>
                  </div>
                  
                  {/* Date & Location tags */}
                  <div className="flex flex-wrap items-center gap-3 text-[#8B7355] text-xs">
                    <span className="flex items-center gap-1 bg-[#FFF6F1] px-2.5 py-1 rounded-full border border-[#E8E2DE] font-medium">
